...
# Suppress yfinance's informational messages for a cleaner log
...
async def fetch_wsj_headlines():
    logging.info("Fetching market headlines from Wall Street Journal...")
    headlines = []
    async with async_playwright() as p:
        try:
            browser = await p.chromium.launch(headless=True)
            page = await browser.new_page()
            await page.goto("https://www.wsj.com/news/markets", timeout=90000)
            try:
                # Handle cookie consent banners which are common
                if await page.locator("#cx-scrim-container").is_visible(timeout=5000):
                    await page.locator("#cx-scrim-container button").first.click()
                    logging.info("Handled cookie consent banner.")
            except Exception: logging.info("No cookie banner found or handled.")
            await page.wait_for_selector("article a[href*='/articles/']", timeout=30000)
            elements = await page.locator("article a[href*='/articles/']").all()
            for el in elements[:5]:
                if title := await el.inner_text():
                     if len(title) > 20: # Filter out short/non-headline links
                        headlines.append({"title": title, "url": await el.get_attribute('href'), "source": "Wall Street Journal"})
            await browser.close()
        except Exception as e:
            logging.error(f"Could not fetch WSJ headlines: {e}")
            
    if headlines: logging.info(f"✅ Fetched {len(headlines)} headlines from WSJ.")
    else: logging.warning("⚠️ WSJ headline fetching failed.")
    return headlines

async def fetch_macro_sentiment(session):
    if not NEWSAPI_KEY:
        logging.warning("NewsAPI key not found, skipping macro analysis.")
        return {"geopolitical_risk": 0, "trade_risk": 0, "economic_sentiment": 0, "overall_macro_score": 0, "geo_articles": [], "trade_articles": [], "econ_articles": []}
    ...
    geo_task = get_news("war OR conflict OR military action OR invasion OR geopolitics")
    trade_task = get_news('"trade war" OR tariffs OR sanctions OR trade dispute')
    econ_task = get_news('"interest rates" OR inflation OR recession OR "gdp growth" OR "jobs report"')
...
async def fetch_context_data(session):
    # FIX: Use correct CoinGecko IDs for futures
    ...
    # Fetch Gold and Silver from a different endpoint if needed or use yfinance as a fallback
    try:
        gold_price = yf.Ticker('GC=F').history(period='1d')['Close'][0]
        silver_price = yf.Ticker('SI=F').history(period='1d')['Close'][0]
...
def generate_html_email(df_stocks, context, market_news, macro_data, memory):
    ...
    def create_context_table(ids):
        rows = ""
        for asset_id in ids:
            if asset_id in context and (asset := context[asset_id]):
    ...
    # Build dynamic editor's note with memory
    ...
    if score_diff > 3: mood_change = f"improved since yesterday (from {prev_score:.1f} to {current_score:.1f})"
    elif score_diff < -3: mood_change = f"turned more cautious since yesterday (from {prev_score:.1f} to {current_score:.1f})"
    ...
    # Build sector deep dive
    sector_html = ""
    if not df_stocks.empty:
        top_by_sector = df_stocks.groupby('sector').apply(lambda x: x.nlargest(2, 'score'), include_groups=False).reset_index()
