...
def fetch_sp500_tickers_sync():
    try: return pd.read_html(StringIO(requests.get("https://en.wikipedia.org/wiki/List_of_S%26P_500_companies", headers=REQUEST_HEADERS, timeout=15).text))[0]["Symbol"].tolist()
...
async def fetch_market_headlines():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await browser.close()
            if headlines:
                logging.info(f"✅ Successfully fetched {len(headlines)} headlines from Google Finance.")
                return headlines
        except Exception as e:
            logging.warning(f"Google Finance scrape failed: {e}. Trying Finviz...")

    # Source 2: Fallback to Finviz
    async with aiohttp.ClientSession() as session:
        content = await make_robust_request(session, "https://finviz.com/")
        if content:
            soup = BeautifulSoup(content, 'html.parser')
            news_list = soup.select('a.nn-tab-link')
            headlines = [{"title": a.text, "url": a['href'], "source": "Finviz"} for a in news_list[:5]]
            if headlines:
                logging.info(f"✅ Successfully fetched {len(headlines)} headlines from Finviz.")
                return headlines

    # Source 3: Final fallback to NewsAPI (if configured)
    if NEWSAPI_KEY:
        logging.warning("All scraping failed. Falling back to NewsAPI for headlines.")
        async with aiohttp.ClientSession() as session:
...
# NEW: Direct data fetch for Crypto and Commodities
...
def generate_html_email(df_stocks, context, market_news, memory):
    ...
    def create_context_table(ids):
        ...
        for asset_id in ids:
            if asset_id in context:
                ...
                price = f"${asset_context.get('current_price', 0):,.2f}"
                change_24h = asset_context.get('price_change_percentage_24h', 0)
                change_7d = asset_context.get('price_change_percentage_7d_in_currency', 0)
                market_cap = f"${asset_context.get('market_cap', 0) / 1_000_000_000:.1f}B"
                color_24h = "#16a34a" if change_24h >= 0 else "#dc2626"
                rows += f'<tr><td style="padding: 10px; border-bottom: 1px solid #eee;"><b>{asset_context["name"]}</b><br><span style="color:#666;font-size:0.9em;">{asset_context["symbol"].upper()}</span></td><td style="padding: 10px; border-bottom: 1px solid #eee;">{price}<br><span style="color:{color_24h};font-size:0.9em;">{change_24h or 0:.2f}% (24h)</span></td><td style="padding: 10px; border-bottom: 1px solid #eee;">{market_cap}<br><span style="font-size:0.9em;">{change_7d or 0:.2f}% (7d)</span></td></tr>'
        return rows
    ...
    sector_html = ""
    if not df_stocks.empty:
        for sector, group in df_stocks.groupby('sector').apply(lambda x: x.nlargest(1, 'score')).groupby('sector'):
            if sector == 'N/A' or sector is None: continue
            row = group.iloc[0]
            sentences, short_summary = row["summary"].split('. '), '. '.join(sentences[:2]) + '.'
            sector_html += f'<div style="margin-bottom: 15px;"><b>{row["name"]} ({row["ticker"]})</b> in <i>{sector}</i><p style="font-size: 0.9em; color: #333; margin: 5px 0 0 0;">{short_summary}</p></div>'
